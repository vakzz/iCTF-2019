...
def print_status():
    ...
    should_beep = False
    for service_id in list(service_states.keys()):
        state = service_states[service_id]["service_state"]
        pwned = any(lambda x: x["team_id"] == TEAM_ID,
                    pwned_states[service_id]["teams"])
    ...
    if "tick" in status:
        tick = status["tick"]
        print(("Round time remaining: {}".format(
            tick["approximate_seconds_left"])))
